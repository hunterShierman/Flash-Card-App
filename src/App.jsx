...
// Keyframes for the deck-of-cards arc transitions
const KEYFRAMES = `
  /* Next: top card arcs back — lifts up, rotates slightly, shrinks behind */
  @keyframes arcToBack {
    0%   { transform: translateY(0px)   rotate(0deg)   scale(1)    translateZ(0px);   opacity: 1; z-index: 10; }
    35%  { transform: translateY(-60px) rotate(-6deg)  scale(0.95) translateZ(40px);  opacity: 1; z-index: 10; }
    65%  { transform: translateY(-60px) rotate(-6deg)  scale(0.88) translateZ(-60px); opacity: 0.7; z-index: 1; }
    100% { transform: translateY(6px)   rotate(-1.5deg) scale(0.94) translateZ(-80px); opacity: 0; z-index: 1; }
  }
  /* Next: card beneath rises up to top */
  @keyframes riseToTop {
    0%   { transform: translateY(8px)   scale(0.94) rotate(1deg);  opacity: 0.5; z-index: 1; }
    40%  { transform: translateY(-10px) scale(0.97) rotate(0.3deg); opacity: 0.85; z-index: 5; }
    100% { transform: translateY(0px)   scale(1)    rotate(0deg);  opacity: 1;   z-index: 10; }
  }

  /* Prev: new card swoops in from back — reverse of arcToBack */
  @keyframes sweepFromBack {
    0%   { transform: translateY(6px)   rotate(-1.5deg) scale(0.94) translateZ(-80px); opacity: 0;   z-index: 1; }
    35%  { transform: translateY(-60px) rotate(-6deg)   scale(0.88) translateZ(-60px); opacity: 0.7; z-index: 5; }
    65%  { transform: translateY(-60px) rotate(-6deg)   scale(0.95) translateZ(40px);  opacity: 1;   z-index: 10; }
    100% { transform: translateY(0px)   rotate(0deg)    scale(1)    translateZ(0px);   opacity: 1;   z-index: 10; }
  }
  /* Prev: current card sinks back down */
  @keyframes sinkToBack {
    0%   { transform: translateY(0px)  scale(1)    rotate(0deg);   opacity: 1;   z-index: 10; }
    40%  { transform: translateY(-10px) scale(0.97) rotate(0.3deg); opacity: 0.85; z-index: 5; }
    100% { transform: translateY(8px)  scale(0.94) rotate(1deg);   opacity: 0.5; z-index: 1; }
  }
`;
...
export default function App() {
  ...
  function goNext() { navigateTo((index + 1) % cards.length, "next"); }
  function goPrev() { navigateTo((index - 1 + cards.length) % cards.length, "prev"); }
  ...
  if (page === "deck") return (
    <Deck onBack={() => setPage("app")} onLoad={(parsed) => { setCards(parsed); reset(); setPage("app"); }} />
  );

  // Which animation applies to which layer during transition
  const topAnim    = dir === "next" ? `arcToBack ${ANIM_MS}ms cubic-bezier(0.4,0,0.2,1) forwards`
                   : dir === "prev" ? `sweepFromBack ${ANIM_MS}ms cubic-bezier(0.4,0,0.2,1) forwards`
                   : "none";
  const bottomAnim = dir === "next" ? `riseToTop ${ANIM_MS}ms cubic-bezier(0.4,0,0.2,1) forwards`
                   : dir === "prev" ? `sinkToBack ${ANIM_MS}ms cubic-bezier(0.4,0,0.2,1) forwards`
                   : "none";

  function CardFace({ card, flippable }) {
    return (
      <div style={{
        width: "100%", height: "100%", position: "relative",
        transformStyle: "preserve-3d",
        transition: flippable ? "transform 0.55s cubic-bezier(0.4,0,0.2,1)" : "none",
        ...
        <div style={{ ...faceStyle, background: "#ffffff", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
          ...
          <div style={{ fontSize: 25, lineHeight: 1.65, color: "#1a3a2a", textAlign: "center", maxWidth: 520 }}>{card.front}</div>
          {flippable && <div style={{ position: "absolute", bottom: 22, fontSize: 11, color: "#a0c4b0", letterSpacing: 2 }}>CLICK TO REVEAL</div>}
        </div>
        <div style={{ ...faceStyle, background: "#f0faf4", boxShadow: "0 20px 60px rgba(0,0,0,0.25)", transform: "rotateY(180deg)", borderTop: "4px solid #2d8a57" }}>
          ...
          <div style={{ fontSize: 25, lineHeight: 1.75, color: "#1a3a2a", textAlign: "center", maxWidth: 540 }}>{card.back}</div>
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ width: "100%", maxWidth: 700, marginBottom: 36 }}>
          ...
          <div style={{ width: "100%", maxWidth: 700, marginBottom: 20, marginTop: 14 }}>
            <button
        ...
        {done ? (
          <div style={{ width: "100%", maxWidth: 700, textAlign: "center", background: "rgba(255,255,255,0.12)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.25)", borderRadius: 20, padding: "64px 40px" }}>
            ...
            <p style={{ color: "rgba(255,255,255,0.7)", marginBottom: 36, fontSize: 15 }}>
              You knew <span style={{ color: "#7effa0", fontWeight: 600 }}>{known.size}</span> cards and flagged <span style={{ color: "#ffb3b3", fontWeight: 600 }}>{unknown.size}</span> for review.
            </p>
            <div style={{ display: "flex", gap: 14, justifyContent: "center" }}>
              <button onClick={reset} style={btn("rgba(255,255,255,0.15)", "#ffffff", "1px solid rgba(255,255,255,0.3)")}>Restart All</button>
              {unknown.size > 0 && <button onClick={retryUnknown} style={btn("rgba(255,255,255,0.9)", "#1a4731")}>Retry {unknown.size} Flagged</button>}
            </div>
          </div>

        ) : current ? (
          <>
            <div style={{ width: "100%", maxWidth: 700, marginBottom: 10, fontSize: 20, color: "#ffffff", textAlign: "right" }}>
              {index + 1} / {cards.length}
            </div>

            {/*
              Deck stage: two layers stacked.
              During "next": top card arcs to back, bottom card rises.
              During "prev": incoming card sweeps from back to top, current sinks.
              At idle: only the top (current) card is visible and flippable.
            */}
            <div
              style={{
                width: "100%", maxWidth: 700, height: 440,
                position: "relative", marginBottom: 26,
                perspective: 1200,
                // slight stacked-deck shadow hints
                filter: cards.length > 1 ? "drop-shadow(3px 6px 0px rgba(0,0,0,0.18)) drop-shadow(6px 10px 0px rgba(0,0,0,0.10))" : undefined,
              }}
            >
              {phase === "animating" && nextIdx !== null ? (
                <>
                  {/* Bottom layer: the card being revealed (next) or departing (prev) */}
                  <div style={{
                    position: "absolute", inset: 0,
                    animation: bottomAnim,
                    transformOrigin: "center bottom",
                    zIndex: dir === "prev" ? 10 : 1,
                  }}>
                    <CardFace card={dir === "next" ? cards[nextIdx] : current} flippable={false} />
                  </div>

                  {/* Top layer: current card arcing away (next) or the prev card arriving */}
                  <div style={{
                    position: "absolute", inset: 0,
                    animation: topAnim,
                    transformOrigin: "center bottom",
                    zIndex: dir === "next" ? 10 : 1,
                  }}>
                    <CardFace card={dir === "next" ? current : cards[nextIdx]} flippable={false} />
                  </div>
                </>
              ) : (
                /* Idle: single flippable card */
                <div
                  onClick={() => setFlipped(f => !f)}
                  style={{ position: "absolute", inset: 0, cursor: "pointer" }}
                >
                  <CardFace card={current} flippable={true} />
                </div>
              )}
            </div>

            {/* Actions */}
            {!phase && flipped ? null : null}
            {phase === "idle" && flipped ? (
              <div style={{ display: "flex", gap: 16 }}>
                <button onClick={() => mark("unknown")} style={btn("rgba(255,100,100,0.2)", "#ffb3b3", "1px solid rgba(255,100,100,0.4)")}>✗ &nbsp; Still Learning</button>
                <button onClick={() => mark("known")} style={btn("rgba(100,255,150,0.2)", "#7effa0", "1px solid rgba(100,255,150,0.4)")}>✓ &nbsp; Got It</button>
              </div>
            ) : (
              <div style={{ display: "flex", gap: 12 }}>
                <button onClick={goPrev} style={btn("rgba(255,255,255,0.15)", "#ffffff", "1px solid rgba(255,255,255,0.25)")}>← Prev</button>
                <button onClick={goNext} style={btn("rgba(255,255,255,0.15)", "#ffffff", "1px solid rgba(255,255,255,0.25)")}>Next →</button>
              </div>
            )}
          </>
        ) : null}
